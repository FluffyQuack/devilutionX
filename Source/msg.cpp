		...
		if (plr[pnum].plractive && myplr != pnum) {
			...
			if (currlevel == plr[pnum].plrlevel) {
				...
				if ((plr[pnum]._pHitPoints >> 6) > 0)
					...
					plr[pnum]._pVar8 = (plr[pnum]._pAnimLen << 1) / gSpeedMod; //Fluffy: Divide by gSpeedMod to get the var's "real" value (TODO: We should double check we're doing the correct changes to var8)
