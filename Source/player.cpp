...
void SetPlrAnims(int pnum)
{
	...
	plr[pnum]._pNWidth = plr[pnum]._pNWidth_c = 96; //Fluffy: Assign width to both combat and casual walking and standing animations
	...
	if (pc == PC_WARRIOR) {
		...
		plr[pnum]._pNWidth = plr[pnum]._pNWidth_c = 112; //Fluffy: Assign width to both combat and casual walking and standing animations
		plr[pnum]._pWWidth = plr[pnum]._pWWidth_c = 112;
...
void CheckPlrSpell(bool mouseClick) //Fluffy: Added mouseClick
{
	...
	if (rspell == SPL_INVALID) {
		if (sgOptions.Gameplay.bNoEquippedSpellIsAttack && (gameSetup_allowAttacksInTown || leveltype != DTYPE_TOWN)) { //Fluffy: If no spell is selected, then default to an attack
