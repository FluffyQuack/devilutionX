...
void InitMonsterGFX(int monst)
{
	...
	for (anim = 0; anim < 6; anim++) {
		...
		if ((animletter[anim] != 's' || monsterdata[mtype].has_special) && frames > 0) {
			...
			if (sgOptions.Graphics.bInitHwIngameRendering) { //Fluffy: Load monster CL2 as SDL textures
				int groupNum = 8;
...
void M_DiabloDeath(int i, BOOL sendmsg)
{
	...
	Monst->_mVar5 = (int)((Monst->_mVar3 - (Monst->_mx << 16)) / (double)dist); //Fluffy: I don't think this and the next value are actually ever referenced after this
...
void ProcessMonsters()
{
	...
	for (i = 0; i < nummonsters; i++) {
		...
		if (Monst->_mFlags & MFLAG_TARGETS_MONSTER) {
			...
			} else if (Monst->_msquelch != 0 && Monst->MType->mtype != MT_DIABLO && Monst->tickCount == 0) { /// BUGFIX: change '_mAi' to 'MType->mtype'
				Monst->_msquelch--;
			}
		}
		do {
...
void FreeMonsters()
{
	...
	for (i = 0; i < nummtypes; i++) {
		...
		for (j = 0; j < 6; j++) {
			...
			if (sgOptions.Graphics.bInitHwIngameRendering) { //Fluffy: Free SDL texture variants
