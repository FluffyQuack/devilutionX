...
void LoadMissileGFX(BYTE mi)
{
	...
	if (mfd->mFlags & MFLAG_ALLOW_SPECIAL) {
		...
		if (sgOptions.Graphics.bInitHwIngameRendering) //Fluffy: Load as SDL texture
...
void MI_Firewall(Sint32 i)
{
	...
	if (missile[i]._mimfnum != 0 && missile[i]._mirange != 0 && missile[i]._miAnimAdd != -1 && missile[i]._miVar2 < 12) {
		if (newGameplayTick && missile[i]._miVar2 == 0) //Fluffy: This action we only want to happen once ever (gSpeedMod related)
...
void MI_Manashield(Sint32 i)
{
	...
	missile[i]._mitxoff = (plr[id]._pxoff * 65536) * gSpeedMod; //Fluffy: Scale based on current speed of the game (TODO: Is this working correctly? Commented this line away seems to do nothing)
	missile[i]._mityoff = (plr[id]._pyoff * 65536) * gSpeedMod;
...
void MI_Etherealize(Sint32 i)
{
	...
	missile[i]._mix = plr[src]._px;
	missile[i]._miy = plr[src]._py;
	missile[i]._mitxoff = (plr[src]._pxoff * 65536) * gSpeedMod; //Fluffy: Scale based on current speed of the game (TODO: Double check this is working correctly)
	missile[i]._mityoff = (plr[src]._pyoff * 65536) * gSpeedMod;
	if (plr[src]._pmode == PM_WALK3) {
...
void MI_FirewallC(Sint32 i)
{
	...
	if (missile[i]._mirange == 0) {
		missile[i]._miDelFlag = TRUE;
	} else if(newGameplayTick) { //Fluffy: Only do this once per 50ms (otherwise gSpeedMod will make the firewall too large)
		int dp = dPiece[missile[i]._miVar1][missile[i]._miVar2];
		assert(dp <= MAXTILES && dp >= 0);
		int tx = missile[i]._miVar1 + XDirAdd[missile[i]._miVar3];
		int ty = missile[i]._miVar2 + YDirAdd[missile[i]._miVar3];
